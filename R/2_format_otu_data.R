...
full_meta <- full_join(IBA_seq_meta , IBA_malaise_meta) %>%
             full_join(IBA_trap_meta) |> 
              dplyr::select(matches("sample_id_ngi|trap|type|date|lat|lon")) %>% 
...
nicheData <- all_data %>% 
                ...
                mutate(placing_date = as.Date(placing_date , format="%d/%m/%Y"),
                       collecting_date = as.Date(collecting_date,format="%d/%m/%Y")) |> 
                mutate(sampling_time = as.numeric(collecting_date - placing_date)) |> 
                group_by(feeding_niche, week_year, trap_id , trap_habitat , 
                         latitude_wgs84 , longitude_wgs84 , sampling_time) |> 
